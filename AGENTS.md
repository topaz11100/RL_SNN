...
* 이 `AGENTS.md` 는 `Theory.md` 에 적힌 내용을 **코드 레벨 작업 단위로 풀어쓴 실행 계획**이다.
...
1. `lif.py` 에 벡터화된 LIF 셀을 구현한다.
   ...
   * 입력: 현재 막전위 `v`, 입력 전류 `I_syn`, 파라미터(τ, v_th, v_reset 등)
...
2. **PPO Actor 손실**
   ...
   * per-event 손실 `L_e^CLIP(θ) = -min( r_e(θ) A_e, clip(r_e(θ), 1-ε, 1+ε) A_e )` 를 구현하고, 배치 평균을 Actor 손실로 사용한다.
...
5. **이미지 단위 미니배치 업데이트**
   ...
   * 이 배치 전체로부터 `L_PPO` 를 계산하고, CLI 인수 `--ppo-epochs` 만큼 미니배치 반복 업데이트를 수행한다.
...
3. **전역 보상 R**
   ...
   * 목표 발화율 보상 `R_sparse`: 평균 발화율 `\bar r` 이 목표율 `ρ_target` 에 가까울수록 높다.
   * 다양성 보상 `R_div`: 에피소드 누적 winner 히스토그램이 균등 분포에 가까울수록 높다.
   * 안정성 보상 `R_stab`: 동일 이미지를 여러 번 보여줄 때 winner 가 바뀌지 않을수록 +1, 바뀌면 -1.
   * 최종 보상: `R = α_sparse R_sparse + α_div R_div + α_stab R_stab`.
   * α 계수와 `ρ_target` 은 모두 CLI 인수로 받는다.
   * **구현 메모 (R_stab)**

     * 안정성 보상을 구현할 때는 MNIST 데이터셋에서 **샘플 인덱스(index)** 를 함께 받아와야 한다.
     * 학습 루프에서 `prev_winner[index]` 형태의 테이블을 유지하여 각 이미지에 대해 “이전에 관찰된 winner 뉴런”을 추적한다.
     * R_stab 는 에피소드가 종료된 시점(이미지 하나의 시뮬레이션 완료 후)에만 계산하며, 시간축 상의 순간 발화율 변화(temporal smoothness)를 직접 보상으로 사용하지 않는다.
...
3. **보상 및 학습 절차**
   ...
   * 전역 보상 R 정의 및 PPO 업데이트 절차는 시나리오 1.1과 동일하다.
...
1. **네트워크 구조**
   ...
   * 입력 784 → 여러 은닉층(256, 128, 64, 32 등) → 출력 10 LIF.
   * 각 시냅스에 정규화된 레이어 인덱스 `l_norm,i` 를 부여한다.

2. **정책 배치 및 상태**

   * 모든 학습 시냅스에 단일 정책 `π_grad` 사용.
   * 상태 벡터에 `l_norm,i` 를 포함한다.
...
4. **보상 정의**
   ...
   * 에피소드 전역 보상: `R = mean_i R_i` (업데이트가 있었던 시냅스에 대해 평균).

5. **산출물**

   * gradient 정렬 지표 곡선: `-(Δw_i^agent + η_align g_i)^2` 평균/분포.
...
1. **맥락 파악 우선**
   ...
   * 항상 `Theory.md` 와 `AGENTS.md` 를 전제로 삼고, 해당 파일 이름과 섹션을 명시적으로 언급한다.
   * 설계와 다르게 보이는 코드를 작성해야 한다면, 먼저 사용자의 승인을 요청한다.
