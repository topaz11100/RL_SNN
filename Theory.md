...
각 시냅스는 pre/post 스파이크 히스토리를 **최근 T 스텝 전체** 길이로 유지하며, 이를 정책 입력 전단 1D-CNN 의 입력으로 사용한다.

* 완전 비지도 실험 1

  * 타임스텝 T_unsup = 100
  * 스파이크 히스토리 배열 길이 K_unsup = 100
  * 한 시냅스 i 의 입력 배열 크기

    * X_spike,i^(unsup)(t) ∈ R^(2×100)
  * 채널 2 는 각각 pre 뉴런, post 뉴런의 스파이크 시퀀스를 의미한다.

* gradient mimicry 실험 2

  * 타임스텝 T_sup = 16
  * 스파이크 히스토리 배열 길이 K_sup = 16
  * 한 시냅스 i 의 입력 배열 크기

    * X_spike,i^(sup)(t) ∈ R^(2×16)
...
* 스파이크 히스토리 배열
  ...
  * 각 시냅스 i 는 pre/post 스파이크 히스토리를 길이 100 의 배열로 유지한다.
  * CNN 입력 크기: X_spike,i^(unsup)(t) ∈ R^(2×100)
...
* 스파이크 히스토리 배열
  ...
  * 각 시냅스 i 는 pre/post 스파이크 히스토리를 길이 16 의 배열로 유지한다.
  * CNN 입력 크기: X_spike,i^(sup)(t) ∈ R^(2×16)
