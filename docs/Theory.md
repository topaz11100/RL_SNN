...
* 상태 (S_k): 현재 네트워크 가중치 (W_k) (또는 그에 의해 유도되는 로컬 상태 분포)
...
* 상태 (S_k): 현재 가중치 (W_k) 및 그로부터 유도되는 로컬 상태 분포
...
이 프로젝트에서는 모든 시나리오에서 **할인율을 (\gamma = 1)** 로 두고, 시간에 따른 보상 분포를 따로 모델링하지 않는다. 대신

* 완전 비지도/준지도(실험 1–3):
  외부 시점 (k) 에 대해 “다음 배치 보상” (R_{k+1}) 을 한 번에 사용한다.
* 완전지도(실험 4):
