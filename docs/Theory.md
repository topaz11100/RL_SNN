...
에서 서로 다르게 설계된다. 특히 pre/post 이벤트에 대해서는 **단일 정책**이 둘 다 처리하며, pre/post 구분은 **이벤트 타입 원핫 벡터**로 로컬 상태에만 반영한다. 별도의 pre 전용, post 전용 정책은 사용하지 않는다. 
...
이미지 간 미니배치를 섞지 않고 **에피소드 단위 on-policy Monte Carlo** 설정을 유지한다. 
...
$$
x^{\text{pre}}_i(\tau) = s_j(t-\tau), \quad \tau = 0, \dots, L-1
$$

$$
x^{\text{post}}_i(\tau) = s_k(t-\tau), \quad \tau = 0, \dots, L-1
$$
...
$$
z_i^{\text{unsup}}(t) = [h_i(t) ; w_i(t) ; \mathbf e_{\text{type}}(e)]
$$

$$
z_i^{\text{semi}}(t) = [h_i(t) ; w_i(t) ; \mathbf e_{\text{type}}(e)]
$$

- 완전지도 gradient mimicry 시나리오:

$$
z_i^{\text{sup}}(t) = [h_i(t) ; w_i(t) ; l_{\text{norm},i} ; \mathbf e_{\text{type}}(e)]
$$
...
- 완전비지도·준지도: $h_i(t)$ 16차원 + $w_i(t)$ 1차원 + $\mathbf e_{\text{type}}(e)$ 2차원 → $d_{\text{unsup}} = d_{\text{semi}} = 19$
- 완전지도: $h_i(t)$ 16차원 + $w_i(t)$ 1차원 + $l_{\text{norm},i}$ 1차원 + $\mathbf e_{\text{type}}(e)$ 2차원 → $d_{\text{sup}} = 20$
...
3. 각 이벤트 $e = (i,t,\text{type})$ 에 대해
   - 로컬 상태 $s_e = z_i(t)$ 를 구성한다. 이때 이벤트 타입 $\text{type}$ 은 원핫 벡터 $\mathbf e_{\text{type}}(e)$ 로 인코딩되어 $z_i(t)$ 에 포함된다.
...
$$
\theta \leftarrow \theta - \eta_{\text{actor}} \nabla_\theta L_{\text{actor}}, \quad
\phi \leftarrow \phi - \eta_{\text{critic}} \nabla_\phi L_{\text{critic}}
$$

과 같이 각각 독립적으로 수행한다. 엔트로피 정규화 항 등은 사용하지 않고, 정책 표준편차 $\sigma_{\text{policy}}$ 를 직접 하이퍼파라미터로 조절한다.
...
2. 각 시냅스 $i$ 및 이벤트 $e = (i,t,\text{type})$ 에 대해 2.3–2.5의 방식으로 상태 $z_i(t)$ 를 만든다. 이때 이벤트 타입 $\text{type}$ 은 $\mathbf e_{\text{type}}(e)$ 로 인코딩된다.
...
$$
H_j^{(K)} = \sum_{k=1}^{K} \mathbf{1}[j^*(k) = j], \quad
...
4. Actor–Critic 업데이트:
   - 해당 에피소드 버퍼에 대해서만 2.9의 수식으로 $L_{\text{actor}}, L_{\text{critic}}$ 를 계산하고 $\theta, \phi$ 를 한 번 업데이트한다.
...
2. 각 뉴런 $j$ 와 각 클래스 $y$ 에 대해 해당 클래스에 속하는 모든 예제에 대한 $r_j(x)$ 를 평균 내어 $\bar r_{j,y}$ 를 계산한다.
3. 뉴런 $j$ 의 라벨을
   $$
...
4. 어떤 입력 $x$ 에 대해 예측할 때는
   ...
   - 각 클래스 $c$ 에 대해 $L_j = c$ 인 뉴런 집합 $\mathcal N_c$ 를 구성한다.
   - 클래스 점수:
     $$
...
로컬 상태 $z_i(t)$ 의 정의는 2.3–2.5와 동일하며, 이벤트 타입은 항상 $\mathbf e_{\text{type}}(e)$ 로 원핫 인코딩되어 포함된다. pre/post 여부와 상관없이, 시냅스가 Input→E 또는 I→E 어디에 속하는지에 따라 $\pi_{\text{exc}}$ 또는 $\pi_{\text{inh}}$ 를 선택한다.
...
3. 에피소드 종료 후 출력층 뉴런 $k$ 에 대해 스파이크 수 $s_k$ 와 발화율 $r_k = s_k / T_{\text{semi}}$ 를 계산한다.
4. 예측 라벨은
   $$
   \hat y = \arg\max_k r_k
   $$
...
2. 마진 기반 보상 $R_{\text{margin}}$:
   ...
   - 정답 뉴런 발화율 $r_y$ 와 오답 중 최대 발화율 $r_{\text{max,wrong}}$ 의 차이를
     $$
...
2. 마진 분포: 마진 $M = r_y - r_{\text{max,wrong}}$ 의 히스토그램을 학습 전·후로 비교해 출력층 뉴런의 분리 정도를 시각화한다.
...
로 정의한다. 여기서 $g_i = \partial \mathcal L_{\text{sup}}/\partial w_i$, $\eta_{\text{align}} > 0$ 는 기준 업데이트 스케일(hyperparameter)이다.
...
1. gradient 정렬 지표 곡선: $-\bigl(\Delta w_i^{\text{agent}} + \eta_{\text{align}} g_i\bigr)^2$ 의 평균 및 분포를 에피소드 수에 따라 기록한다.
...
2. 신뢰할당 문제와 gradient mimicry 의 역할  
   비지도·준지도 시나리오에서는 에피소드 단위 전역 보상만 존재하여 전형적인 신뢰할당 문제가 나타난다. gradient mimicry 시나리오에서는 Teacher gradient 를 통해 좋은 업데이트 방향과 크기에 대한 정보를 직접 제공받아 신뢰할당 문제를 어느 정도 완화할 수 있는지 살펴본다.
3. 마진과 뉴런 역할 분화  
...
1. 에피소드 단위 전역 보상에 따른 신뢰할당 문제  
   모든 시나리오에서 이미지 1장에 대해 전역 보상 $R$ 하나만 계산하고 에피소드 내 모든 이벤트에 동일한 보상을 할당한다. 이 때문에 어떤 이벤트가 보상을 개선 또는 악화시켰는지를 세밀하게 구분하기 어렵고, 전형적인 Monte Carlo 기반 Actor–Critic 과 같은 신뢰할당 문제가 존재한다.
