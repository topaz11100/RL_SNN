...
- `build_parser() -> argparse.ArgumentParser`
  - **인수**: 없음.
  - **역할**: 시나리오 선택(`unsup1/unsup2/semi/grad`), 타임스텝(`T_*` 기본 100), 스파이크 히스토리 길이(`L` 기본 20)와 PPO/보상/클리핑 하이퍼파라미터를 한 곳에서 받는 파서를 생성한다. `--events-per-image`(기본 1024)로 이미지당 이벤트 수를 제한하고 `--event-batch-size`(기본 1024)로 Actor/Critic 추론 미니배치 크기를 조절한다.
  - **추가 인자**: 로그 주기(`--log-interval`), 결과/로그 디렉터리 이름(`--run-name`), 레이어 정규화 스케일(`--layer-index-scale`), gradient mimicry 입력 모드(`--direct-input`), 추가 통계 로깅(`--log-gradient-stats`) 등 각 시나리오별 보조 플래그를 노출한다. 지도/준지도 가중치 클립은 `--w-clip-min/max`, 흥분/억제 클립은 `--exc-clip-*`/`--inh-clip-*`로 설정한다.
  - **출력**: 구성된 `ArgumentParser` 객체.
  - **Theory 연계/최적화**: `Theory.md` 2.9절의 PPO 설정(σ, ε, 에폭, 미니배치)을 모두 인자로 노출하고, GPU 리소스 절약을 위해 이벤트/ppo 미니배치 기본값을 넉넉히 잡아 긴 시퀀스에서도 안정적인 업데이트를 유지한다.
...
- `resolve_path(path: Union[str, Path]) -> Path`
  - **인수**: 문자열 또는 `Path` 객체.
  - **역할**: 상대경로 입력 시 프로젝트 루트를 기준으로 절대경로로 변환해 로그/결과 파일 생성 시 경로 혼동을 방지한다.
  - **출력**: 절대 `Path` 객체.
- `create_result_dir(scenario: str, run_name: Optional[str] = None) -> str`
  ...
  - **출력**: 생성된 디렉터리 경로 문자열.
  - **Theory 연계**: 모든 시나리오가 동일한 결과 폴더 구조를 사용하도록 강제해 로그/메트릭 정리를 단순화한다.
...
- `_ensure_eval_file(path: str | Path) -> None`
  - **역할**: 검증 정확도 기록을 위한 별도 평가 파일을 생성(없을 때만 헤더 작성).
