...
def plot_delta_t_delta_d(delta_t: torch.Tensor | np.ndarray, delta_d: torch.Tensor | np.ndarray, out_path: str) -> None:
    ...
    np.savez(base + ".npz", delta_t=dt_np, delta_d=dd_np)
...
def plot_weight_histograms(
    ...
    np.savez(base + ".npz", weights_before=w_before, weights_after=w_after)
    with open(base + "_stats.txt", "w") as f:
        f.write(
            "before_mean\t{:.6f}\nbefore_std\t{:.6f}\nafter_mean\t{:.6f}\nafter_std\t{:.6f}\n".format(
...
def evaluate_labeling(
    ...
    np.savez(out_prefix + ".npz", confusion=conf, preds=preds_np, labels=labels_np)
    with open(out_prefix + "_metrics.txt", "w") as f:
...
def plot_grad_alignment(
    ...
    np.savez(base + ".npz", delta_agent=d_agent, delta_teacher=d_teacher)
